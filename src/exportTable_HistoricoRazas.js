...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Object.values(byCcaa).forEach(group => {
    group.sort((a, b) => b.rating - a.rating);
    group.forEach((row, idx) => { row.ccaaRank = idx + 1; });
  });

  // Población de filtros
  ccaaFilter.innerHTML = '<option value="all">Todas</option>' +
    Array.from(new Set(allRows.map(r => r.ccaa))).sort()
  ...
  [ccaaFilter, raceFilter, wrMinFilter, wrMaxFilter, gamesMinFilter, gamesMaxFilter]
