...
// filtra solo entrenadores con Country Spain, expande estadísticas por raza históricas,
// excluye partidas 0, aplica filtros de CCAA, Raza, NAF, Entrenador, WR y Partidos,
// asigna ranking estático global, por raza y por CCAA, y renderiza la tabla con
// posiciones dinámicas solo si se filtra por raza.

document.addEventListener("DOMContentLoaded", () => {
  if (typeof statsData === "undefined") {
    console.error(
      "statsData no está definido. Asegúrate de que statsData.js se cargue antes."
    );
    return;
  }
  ...
  // Construir todas las filas de datos para Country Spain
  const allRows = [];
  statsData.forEach((item) => {
    if ((item.Country || "").trim() !== "Spain") return;
    const base = {
      nafNr: item["NAF Nr"] || "",
      coach: item["NAF Name"] || "",
      ccaa: item.CCAA || "",
  ...
  // Ranking estático por CCAA
  const byCcaa = allRows.reduce((acc, row) => {
    (acc[row.ccaa] = acc[row.ccaa] || []).push(row);
    return acc;
  }, {});
  Object.values(byCcaa).forEach((group) => {
    group.sort((a, b) => b.rating - a.rating);
    group.forEach((row, idx) => {
      row.ccaaRank = idx + 1;
    });
  ...
  ccaaFilter.innerHTML =
    ...
    Array.from(new Set(allRows.map((r) => r.ccaa)))
      .sort()
  ...
  raceFilter.innerHTML =
    '<option value="all">Todas</option>' +
    Array.from(new Set(allRows.map((r) => r.race)))
      .sort()
  ...
  function applyFilters() {
    ...
    let gmMin = -Infinity,
      gmMax = Infinity;
    const gmin = gamesMinFilter.value,
      gmax = gamesMaxFilter.value;
    if (gmin)
      gmMin = gmin.endsWith("+") ? parseInt(gmin, 10) : parseInt(gmin, 10);
    if (gmax) gmMax = gmax.endsWith("+") ? Infinity : parseInt(gmax, 10);

    // Filtrar según controles
    let filtered = allRows.filter((r) => {
      if (cc !== "all" && r.ccaa !== cc) return false;
    ...
    filtered.forEach((row) => {
      ...
      // CCAA rank dinámico cuando se filtra por raza, si no estático
      row.displayCcaaRank =
        rf !== "all"
          ? (() => {
              // agrupa por CCAA dentro de esta raza y asigna rank dinámico
              const group = filtered.filter((r) => r.ccaa === row.ccaa);
              group.sort((a, b) => b.rating - a.rating);
    ...
    filtered.forEach((r) => {
      ...
      tr.innerHTML = `
        ...
        <td>${r.nafNr}</td>
        <td>${r.coach}</td>
        <td>${r.ccaa} (${r.displayCcaaRank})</td>
        <td>${r.race}</td>
        <td>${r.tournaments}</td>
