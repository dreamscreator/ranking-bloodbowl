// tableBbtProplayers.js (vista BBT Proplayers)
//
// - Carga datos desde proplayers.js (variable proplayersBbt)
// - Usa general_all.js (variable generalAllBbt) para "Total partidos" (games)
// - Calcula % vs proplayers = (ppGames + topGames + megaGames) / games * 100
// - Renderiza columnas de bbtProplayers.html (id="nafTable")
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof proplayersBbt === "undefined") {
        console.error("proplayersBbt no está definido. Carga src/bbt/proplayers.js antes de este script.");
        return;
    }
    const hasGeneral = typeof generalAllBbt !== "undefined";
    ...
    // Normalizar filas desde proplayersBbt
    const data = (proplayersBbt || []).map((p) => {
        const nafNr = String(p["NAF Nr"] || "");
        const coach = p["NAF Name"] || "";
        // En BBT a veces el campo Country depende de cómo se genere, aquí asumimos que viene
        ...
        // Total partidos globales del entrenador (generalAllBbt) si disponible
    ...
    function populateFilterOptions() {
        if (ccaaFilter) {
            const list = Array.from(new Set(data.map((item) => item.ccaa))).filter(Boolean).sort();
