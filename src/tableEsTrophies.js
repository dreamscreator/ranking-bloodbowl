(function () {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // âœ… ahora es CCAA (con fallback por si no cambias el html por error)
    const ccaaFilter =
      document.getElementById("ccaaFilter") || document.getElementById("countryFilter");
    ...
    // âœ… Premios desde item.trophies.* + âœ… filtro SOLO Spain
    // âœ… guardamos tambiÃ©n CCAA
    const baseData = source
      .map((item) => {
        const trophies = item?.trophies || {};
        const country = item["Country"] ?? item.country ?? "";
        const ccaa = item["CCAA"] ?? item.ccaa ?? "";
      ...
      .filter((row) => normCountry(row.country) === "spain"); // âœ… SOLO Spain
    ...
    function renderTable(rows) {
      ...
      rows.forEach((r) => {
        ...
        tr.innerHTML = `
          <td>${r.nafNr}</td>
          <td>${r.coach}</td>
          <td class="country-column">${r.ccaa || ""}</td>
          <td>${r.winner}</td>
          <td>${r.runnerup}</td>
          <td>${r.touchdowns}</td>
          <td>${r.casualties}</td>
